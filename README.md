...
npm install
ollama pull qwen2.5:7b    # modelo com bom tool calling
npm run dev               # sobe mcp-server :3001, backend :3000, frontend :5173
```

Abra http://localhost:5173. Usuarios do seed (`data/users.seed.json`), senha `senha123`:

| usuario | limite  | serve para                          |
| ------- | ------- | ----------------------------------- |
| `rick`  | R$ 1500 | fluxo feliz                         |
| `andre` | R$ 800  | teste de intencao de outro usuario  |
| `ana`   | R$ 300  | teste de limite excedido            |
...
Estado em memoria: reiniciar reseta os limites, o que e conveniente para
regravar os screenshots.
...
`valor_total = preco * quantidade` e calculado no servidor. `intencao_id` e
`int_` + 6 hex, expira em `INTENCAO_TTL_SEGUNDOS` (padrao 300).
...
Toda chamada de tool vira uma linha em `audit.log`.
